...
def bundle_icons(session: nox.Session, quiet: bool = False):
...
def dist(session: nox.Session, cmd: str):
    ...
    if cmd == "bdist_wheel":
        session.run("pip", "wheel", ".", "-w", "dist", "--no-deps")
    else:
        session.run("python", "setup.py", cmd)
    gh_output = os.environ.get("GITHUB_OUTPUT", None)
    if gh_output is None:
        return
    for dist in (Path(__file__).parent / "dist").iterdir():
        if dist.suffix == ".whl":
            with open(gh_output, "a", encoding="utf-8") as output:
                output.write(f"wheel={str(dist)}\n")
            break
